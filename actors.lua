...
function ball.wall_collision(b)
	if b.pos.y < b.radius then
		return true, vector.new(0, 1)
	elseif b.pos.y > love.graphics.getHeight() - b.radius - 1 then
		return true, vector.new(0,-1)
	elseif b.pos.x < b.radius then
		return true, vector.new( 1,0)
	elseif b.pos.x > love.graphics.getWidth() - b.radius then
		return true, vector.new(-1,0)
	end
	return false
end
		...
		if collides then
			if normal.y == 0 then
				...
				if b.pos.y + b.radius >= love.graphics.getHeight() - 1 then
					b.pos.y = love.graphics.getHeight() - b.radius - 2
				end
			end
			return
		end
